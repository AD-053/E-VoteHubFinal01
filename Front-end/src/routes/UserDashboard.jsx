...
const EditPostModal = ({ post, isOpen, onClose, onSave, isLoading }) => {
  ...
  const handleSave = () => {
    if (!editContent.trim() && existingPictures.length === 0 && existingVideos.length === 0 && editPictures.length === 0 && editVideos.length === 0) {
      Swal.fire({title: 'Invalid Post', text: 'Your post cannot be empty', icon: 'error', confirmButtonText: 'OK', background: 'rgb(255, 0, 0)', color: 'var(--text-primary)'})
      return
    }
    onSave({
...
export default function UserDashboard(){
  ...
  const onRegisterVoter = useCallback(async (e)=>{
    ...
    try{ 
      ...
      Swal.fire({title: 'Registration Successful!', text: 'You have been registered as a voter', icon: 'success', confirmButtonText: 'Great!', background: 'rgb(28, 219, 50)', color: 'var(--text-primary)'}) 
    }catch(err){ 
      Swal.fire({title: 'Registration Failed', text: err?.response?.data?.message || 'Failed to register as voter', icon: 'error', confirmButtonText: 'Try Again', background: 'rgb(255, 0, 0)', color: 'var(--text-primary)'}) 
    }
  }, [activeEvent])
  ...
  const onNominate = useCallback(async (e)=>{
    if(e) { e.preventDefault(); e.stopPropagation() }
    if(!activeEvent) return
    if(!selectedBallot){ Swal.fire({title: 'Missing Ballot', text: 'Please select a ballot image to continue', icon: 'warning', confirmButtonText: 'OK', background: 'rgb(225, 247, 56)', color: 'var(--text-primary)'}); return }
    try{
      ...
      Swal.fire({title: 'Submitted Successfully!', text: 'Your nominee registration has been submitted and is awaiting admin approval', icon: 'success', confirmButtonText: 'Perfect!', background: 'rgb(28, 219, 50)', color: 'var(--text-primary)'})
    }catch(err){
      Swal.fire({title: 'Registration Failed', text: err?.response?.data?.message || 'Failed to register as nominee', icon: 'error', confirmButtonText: 'Try Again', background: 'rgb(255, 0, 0)', color: 'var(--text-primary)'})
    }
  ...
  const onVote = useCallback(async (e)=>{
    ...
    if(requiresCode){
      ...
      if(normalized.length !== 6 || !/^[0-9]{6}$/.test(normalized)){
        Swal.fire({title: 'Invalid Code', text: 'Please enter a valid 6-digit code to submit your vote', icon: 'error', confirmButtonText: 'OK', background: 'rgb(255, 0, 0)', color: 'var(--text-primary)'})
        return
      }
    }
    try{
      ...
      setHasVoted(true)
      Swal.fire({title: 'Vote Submitted!', text: 'Your vote has been successfully submitted and recorded', icon: 'success', confirmButtonText: 'Thank You!', background: 'rgb(28, 219, 50)', color: 'var(--text-primary)'})
    }catch(err){
      Swal.fire({title: 'Vote Failed', text: err?.response?.data?.message || 'Failed to submit your vote', icon: 'error', confirmButtonText: 'Try Again', background: 'rgb(255, 0, 0)', color: 'var(--text-primary)'})
    } finally {
  ...
  const handleSaveEditPost = useCallback(async (editData) => {
    ...
    try {
      ...
      formData.append('removePictureIds', JSON.stringify(
        editingPost.picture.filter(p => !editData.existingPictures.find(ep => ep.publicId === p.publicId)).map(p => p.publicId)
      ))
      formData.append('removeVideoIds', JSON.stringify(
        editingPost.video.filter(v => !editData.existingVideos.find(ev => ev.publicId === v.publicId)).map(v => v.publicId)
      ))
      editData.newPictures.forEach(pic => formData.append('pictures', pic))
      editData.newVideos.forEach(vid => formData.append('videos', vid))

      await apiClient.patch('/api/V1/campaign/posts', formData)
      const refreshed = await listCampaignPosts(activeEvent._id)
      setCampaignPosts(Array.isArray(refreshed)? refreshed : (refreshed?.posts||[]))
      setEditingPostId(null)
      setEditingPost(null)
      Swal.fire({title: 'Success!', text: 'Your post has been updated successfully', icon: 'success', confirmButtonText: 'Done', background: 'rgb(28, 219, 50)', color: 'var(--text-primary)'})
    } catch (err) {
      setCampaignError(err?.response?.data?.message || 'Failed to update post')
      Swal.fire({title: 'Update Failed', text: err?.response?.data?.message || 'Failed to update your post', icon: 'error', confirmButtonText: 'Retry', background: 'rgb(255, 0, 0)', color: 'var(--text-primary)'})
    } finally {
  ...
  const requestVoteCode = useCallback(async () => {
    try{
      ...
      Swal.fire({title: 'Code Sent!', text: 'A verification code has been sent to your email address', icon: 'success', confirmButtonText: 'OK', background: 'rgb(28, 219, 50)', color: 'var(--text-primary)'})
    }catch(err){
      Swal.fire({title: 'Failed to Send', text: err?.response?.data?.message || 'Failed to send verification code', icon: 'error', confirmButtonText: 'Try Again', background: 'rgb(255, 0, 0)', color: 'var(--text-primary)'})
    }finally{
